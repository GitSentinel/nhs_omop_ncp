# Agentic Pipeline Report — 20260804T213934Z

- Target: `original`
- Mode: `post-finetune`
- Started: `2026-08-04T21:39:34.565780+00:00`
- Finished: `2026-08-04T21:44:19.862915+00:00`
- Training GPUs: `[]`
- Evaluation GPU: `0`
...
This is a synthetic research assessment of the original target in post-finetune mode. The pipeline segment corresponding to the original task completed with an evaluation step (original_evaluate) that produced quantitative performance metrics for the base and fine-tuned models. The evidence shows substantial improvements when moving from the base to the fine-tuned model on the original (binary treatment-event) task across key evaluation dimensions. The PIFU (three-class eligibility) results are also present in the evidence but are not the primary target of this assessment and have been explicitly distinguished from the original binary task for interpretation. All outputs are provided for synthetic research purposes and require human review before any real-world extrapolation.
...
| original_evaluate | original | succeeded | 263.5 | 0 |
...
- Original_evaluate completed successfully for the original target. Base vs Fine-Tuned comparison indicates substantial performance gains after finetuning across multiple metrics.
- Delta (Fine-Tuned minus Base): f1 +0.1179; macro_f1 +0.3533; accuracy +0.2053; balanced_accuracy +0.2843; precision +0.1813; recall +0.0078; roc_auc +0.3350; pr_auc +0.1779.
- Per-class and confusion-matrix details corroborate improved discrimination with the fine-tuned model (e.g., reduction in false negatives from 15 to 12 in the provided confusion matrices).
- Notable improvements are most pronounced in macro- and PR-based metrics (Macro F1, PR AUC) indicating better class-agnostic performance after finetuning.
- PIFU/eligibility metrics are included in the evidence but target is the original binary treatment-event task; classification of NOT_ELIGIBLE, BORDERLINE, and ELIGIBLE in the PIFU tasks should be interpreted separately and do not directly affect the reported binary-task results.
...
- Document the strong improvements observed with finetuning for the original task and consider further validation on independent synthetic datasets.
- Review the PIFU eligibility metrics separately if a joint analysis is intended, ensuring clear separation from the binary treatment-event results.
- If this analysis informs deployment decisions, perform a sensor pass to verify numeric stability across multiple seeds and data slices; report any potential bias or distribution shift in future iterations.

## Comparison

Base vs Fine-Tuned comparison (original task): f1 improved from 0.7831 to 0.9010 (+0.1179); macro_f1 from 0.4864 to 0.8397 (+0.3533); accuracy from 0.6578 to 0.8631 (+0.2053); balanced_accuracy from 0.5366 to 0.8209 (+0.2843); precision from 0.6607 to 0.8420 (+0.1813); recall from 0.9610 to 0.9688 (+0.0078); roc_auc from 0.6214 to 0.9564 (+0.3350); pr_auc from 0.7948 to 0.9727 (+0.1779). The COMPARISON block in the evidence confirms these deltas for the base vs fine-tuned models.
...
[
  ...
  },
  {
    ...
    "source": "/home/jacinth/nhs_omop_ncp/data/pipeline_runs/20260804T213934Z/original_evaluate.log"
  },
  {
    "dataset": "original",
    "model": "fine_tuned",

...
def get_person(person_id: int) -> dict:
    ...
    result = person.filter(person.person_id == person_id).execute()

    if result.empty:
        raise ValueError(f"No patient found with person_id={person_id}.")
...
def get_conditions(person_id: int) -> list[dict]:
    ...
    query = (
        conditions.filter(conditions.person_id == person_id)
        .left_join(concept, conditions.condition_concept_id == concept.concept_id)
        .select(
...
def get_medications(person_id: int) -> list[dict]:
    ...
    query = (
        medications.filter(medications.person_id == person_id)
        .left_join(concept, medications.drug_concept_id == concept.concept_id)
        .select(
...
def get_visits(person_id: int) -> list[dict]:
    ...
    query = (
        visits.filter(visits.person_id == person_id)
        .left_join(concept, visits.visit_concept_id == concept.concept_id)
        .select(
...
def get_measurements(person_id: int, limit: int = 50) -> list[dict]:
    ...
    query = (
        measurements.filter(measurements.person_id == person_id)
        .left_join(concept, measurements.measurement_concept_id == concept.concept_id)
        .select(
...
def get_observations(person_id: int) -> list[dict]:
    ...
    query = (
        observations.filter(observations.person_id == person_id)
        .left_join(concept, observations.observation_concept_id == concept.concept_id)
        .select(
...
def get_notes(person_id: int) -> list[dict]:
    ...
    query = (
        notes.filter(notes.person_id == person_id)
        .select(
            notes.note_id,
            notes.note_date,
...
def get_procedures(person_id: int) -> list[dict]:
    ...
    query = (
        procedures.filter(procedures.person_id == person_id)
        .left_join(concept, procedures.procedure_concept_id == concept.concept_id)
        .select(
